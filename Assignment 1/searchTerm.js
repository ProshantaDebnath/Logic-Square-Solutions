function findCaliforniaCafes(searchItem) {
    ...
    //search action to take input
    const filteredCafes = cafes.filter((cafe) =>
      cafe.name.toLowerCase().includes(searchItem.toLowerCase())
    );
  
    let results = []; //variable to store filter data

    //implemention to perform search
    for (let i = 0; i < filteredCafes.length; i++) {
      for (let j = 0; j < places.length; j++) {
        if (filteredCafes[i].place_id === places[j].id) {
          const tempObj = { ...filteredCafes[i], ...places[j] };
          delete tempObj.place_id && delete tempObj.id;
  ...
  //search key--->
  console.log(findCaliforniaCafes("Hi-Lo Cafe"));
